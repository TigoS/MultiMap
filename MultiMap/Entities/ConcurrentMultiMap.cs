        ...
        public bool Equals(IReadOnlyMultiMap<TKey, TValue>? other)
        {
            ...
            try
            {
                ...
                foreach (var kvp in _dictionary)
                {
                    ...
                    HashSet<TValue> thisSet;
                    lock (kvp.Value)
                    {
                        thisSet = new HashSet<TValue>(kvp.Value, _valueComparer);
                    }

                    var otherValues = other[kvp.Key];
